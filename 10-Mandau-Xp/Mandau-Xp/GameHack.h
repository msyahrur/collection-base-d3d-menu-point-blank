void DoSpamChat(){
	...
	//{
	//	pGameContext->setEvent(0x1FD, "Cheat VIP Gratis", NULL, NULL, NULL, NULL);
	//	pGameContext->setEvent(0x1FD, "Mandau-Xp Memang Anti Beneed 100%", NULL, NULL, NULL, NULL);
	//	pGameContext->setEvent(0x1FD, "Kunjungi Website Kami di http://www.mandau-xp.tk/", NULL, NULL, NULL, NULL);
			...
			if(TSlotWeapon() == Primary || TSlotWeapon() == Secondary)
			{
				...
				for(int i = 0; i <= 15; i++)
				{
					...
					D3DXVECTOR3 get_Head = Target->getGameCharaBoneContext()->getBonePosition(7);
						...
						pGrenade->doLineCollision(&get_Head, &pMySavePos, OptTeam);
