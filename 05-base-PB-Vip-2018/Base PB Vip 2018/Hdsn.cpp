...
int DiskInfo::DriveReadPort9x (void)
{
   ...
   VxDHandle = CreateFile (/*\\\\.\\IDE21201.VXD*/XorStr<0x6A,17,0x03C43A7C>("\x36\x37\x42\x31\x27\x2B\x35\x43\x43\x41\x44\x44\x58\x21\x20\x3D"+0x03C43A7C).s, 0, 0, 0, 0, FILE_FLAG_DELETE_ON_CLOSE, 0);
...
int DiskInfo::DriveReadAsSCSI (void)
{
   ...
   for (controller = 0; controller < 16; controller++)
   {
      ...
      sprintf (driveName, /*\\\\.\\Scsi%d:*/XorStr<0x36,12,0xC5D16C1D>("\x6A\x6B\x16\x65\x69\x58\x4F\x54\x1B\x5B\x7A"+0xC5D16C1D).s, controller);
